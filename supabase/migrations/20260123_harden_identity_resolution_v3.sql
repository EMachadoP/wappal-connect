-- =====================================================
-- HARDEN IDENTITY RESOLUTION V3 (Production-Grade)
-- =====================================================
-- Refined with strict null-handling, distinct aggregation,
-- parameter normalization, and length-guard for chat_keys.
-- Compat: Returns 'out_chat_key' to match zapi-webhook.
...
BEGIN
  ...
  IF v_lid IS NOT NULL THEN
    ...
    IF v_existing_id IS NOT NULL THEN
      SELECT c.chat_key INTO v_chat_key FROM public.contacts c WHERE c.id = v_existing_id;
      RETURN QUERY SELECT v_existing_id, v_chat_key, 'lid'::text;
      RETURN;
    END IF;
  END IF;
  ...
  IF v_chat_key IS NOT NULL AND length(v_chat_key) < 10 THEN
    RETURN QUERY SELECT NULL::uuid, v_chat_key, 'chat_key_too_short_refuse'::text;
    RETURN;
  END IF;
  ...
  IF v_chat_key IS NOT NULL THEN
    ...
    IF v_existing_id IS NOT NULL THEN
      ...
      SET chat_lid = COALESCE(v_chat_lid, chat_lid),
          lid      = COALESCE(v_lid,      lid),
          name     = CASE WHEN name IS NULL OR name ~ '^\d+$' THEN COALESCE(v_name, name) ELSE name END,
          updated_at = now()
      WHERE id = v_existing_id;

      RETURN QUERY SELECT v_existing_id, v_chat_key, 'chat_key_exact'::text;
      RETURN;
    END IF;
    ...
    IF v_chat_key LIKE '55%' AND (length(v_chat_key)=12 OR length(v_chat_key)=13) THEN
      ...
      IF array_length(v_candidates,1) = 1 THEN
        ...
        RETURN QUERY SELECT v_existing_id, v_chat_key, 'chat_key_variant_unique'::text;
        RETURN;
      ELSIF array_length(v_candidates,1) > 1 THEN
        -- Ambiguous: do NOT guess (prevents mixing contact histories)
        RETURN QUERY SELECT NULL::uuid, v_chat_key, 'ambiguous_variant_refuse'::text;
        RETURN;
      END IF;
    END IF;
    ...
    RETURN QUERY SELECT v_existing_id, v_chat_key, 'created_by_chat_key'::text;
    RETURN;
  END IF;

  -- 5) No stable key available (refuse to create ghost contacts)
  RETURN QUERY SELECT NULL::uuid, NULL::text, 'no_stable_key_refuse'::text;
END;
$$;
