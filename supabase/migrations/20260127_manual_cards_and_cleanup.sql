...
CREATE OR REPLACE FUNCTION delete_plan_item(p_item_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Se tinha work_item, voltar status para 'open'
  ...
  RETURN jsonb_build_object('success', true, 'deleted_work_item_id', v_work_item_id);
END;
$$;

-- 5. Permissões
GRANT EXECUTE ON FUNCTION delete_plan_item(UUID) TO authenticated, service_role;
